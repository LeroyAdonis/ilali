...
/** Check if a date is within 7 days of now */
function isNewMember(joinedAt: Date | null): boolean {
  if (!joinedAt) return false;
  const now = Date.now();
  const sevenDays = 7 * 24 * 60 * 60 * 1000;
  return now - joinedAt.getTime() < sevenDays;
}
      ...
      {memberships.length > 0 ? (
        <div className="grid gap-4 sm:grid-cols-2">
          {memberships.map((member) => {
            ...
            const isAlumni =
              "status" in member && (member as Record<string, unknown>).status === "inactive";
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-1.5">
                      ...
                      {isNew && (
                        <span className="inline-flex items-center rounded-full bg-ilali-50 px-2 py-0.5 text-[10px] font-semibold text-ilali-600">
                          New
                        </span>
                      )}
                      {isAlumni && (
                        <span className="inline-flex items-center rounded-full bg-ink/5 px-2 py-0.5 text-[10px] font-semibold text-ink-faint">
                          Alumni
                        </span>
                      )}
                    </div>
