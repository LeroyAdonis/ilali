...
export default function VenueCard({ venue }: VenueCardProps) {
  ...
  return (
    <Link
      href={`/venues/${slug}`}
      className="group block rounded-xl border border-ink/10 bg-white shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 overflow-hidden"
    >
      {/* Image / Placeholder */}
      <div className="relative h-32 w-full overflow-hidden bg-gradient-to-br from-slate-200 to-slate-300">
        ...
        {/* Type badge */}
        <span className="absolute top-2 left-2 rounded-full bg-white/90 px-2 py-0.5 text-[10px] font-medium capitalize text-ink-soft shadow-sm backdrop-blur-sm">
          {type}
        </span>
      </div>
      ...
      <div className="p-3">
        ...
        <div className="mt-1.5 flex items-center gap-1 text-xs text-ink-faint">
